...
为响应“成长型分身产品应具备可解释和可控机制”的建议，项目补充了以下公开说明页面：

- `lifecycle.html`：说明核心角色、分身、群聊的生命周期状态与触发机制；
- `privacy.html`：隐私政策草案（收集范围、使用目的、用户控制权）；
- `trust-controls.html`：记忆开关、导出/清空、删除说明与“为什么生成分身”解释入口。

> 说明：这些页面是当前静态站点层面的透明化入口，后续可进一步与应用内设置与操作按钮联动。
